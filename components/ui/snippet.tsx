...
export const Snippet = ({
  ...
  return (
    <div
      className={`flex rounded-md border border-gray-alpha-400 px-3 py-2.5 ${colors.background}`}
      style={{ width }}
    >
      <div className="mr-3">
        {content.map((item) => (
          <div
            key={item}
            className={`font-mono text-[13px] ${prompt ? "before:content-['$_']" : ""} ${colors.text}`}
          >
            {item}
          </div>
        ))}
      ...
      <div className="relative ml-auto cursor-pointer">
        <svg
          ...
          viewBox="0 0 16 16"
          width="16"
          className={`absolute right-0 z-10 ${content.length > 1 ? "top-0" : "top-1/2 -translate-y-1/2"} ${colors.fill}${animation ? " animate-fade-out" : ""}`}
          onClick={onClick}
        >
          <path
            fillRule="evenodd"
